...
// ─── Art assets ─────────────────────────────────────────────────────────────
// Drop your image paths/URLs here. Both are optional — set to "" to disable.
//
// ART_PAD_BG: texture/painting shown behind the play field.
//   Recommended: dark atmospheric piece, portrait orientation.
//   Overlaid with the violet radial gradient at ~60% opacity so it integrates
//   naturally without blowing out the UI. High contrast art works best.
//   Example: "/art/pad-bg.jpg"
//
// ART_RAIL_DECO: decorative strip shown above the button rail.
//   Recommended: horizontal banner, ornamental border, or your signature.
//   Rendered full-width at 32px height, object-fit: cover.
//   Example: "/art/rail-deco.png"
const ART_PAD_BG    = "/art/pad-bg.jpg";   // ← drop IMG_7082.JPG into public/art/ as pad-bg.jpg
const ART_RAIL_DECO = "";                   // ← rail decoration art path (optional)
const ART_LYRA      = "/art/lyra.jpg";      // ← drop IMG_7083.JPG into public/art/ as lyra.jpg
...
const INK      = "#1a1410";
const PANEL    = "#241b12";
const SAGE     = "#71A39F"; // autoharp / chords accent
const SAGE_DK  = "#3d6460"; // darker sage for panel backgrounds
// distinct hues for the up-to-5 symphony voices
...
// ─── Audio Engine (polyphonic, up to 5 voices) ────────────────────────────────
function createAudioEngine() {
  let ctx = null, started = false, master = null;
  const MAXV = 5;
  ...
  let fx = null; // { delay, chorus, phaser, reverb } node bundles
  ...
  // Force-resume helper for user-gesture entry points (autoplay unlock).
  function unlock() {
    const c = getCtx();
  ...
  function ensureBuses(){
    ...
    const mkChorusLine=(baseMs,rate,phase)=>{
    ...
    const chL=mkChorusLine(18,0.6,0), chR=mkChorusLine(23,0.5,1);
    chWet.connect(chOut);

    // ── PHASER ── 4 all-pass filters swept by an LFO
    ...
    for(let i=0;i<4;i++){ const ap=c.createBiquadFilter(); ap.type="allpass"; ap.frequency.value=400+i*250; prev.connect(ap); prev=ap; aps.push(ap); }
    ...
    // ── PRESENCE shelf — always active, sits before outGain ──
    ...
    //        dryIn → shelf → outGain
    dryIn.disconnect(); dryIn.connect(shelf);
    ...
    master = wetIn; // live voices feed the wet bus by default
    busesReady = true;
  }

  // ── Effect parameter setters (mix 0..1, plus per-effect params) ──
  function setDelay({mix,time,feedback}){ if(!fx)return; const c=getCtx(),n=c.currentTime;
    if(mix!=null){ fx.delay.wet.gain.setTargetAtTime(mix,n,0.05); fx.delay.dry.gain.setTargetAtTime(1-mix*0.0,n,0.05); }
  ...
  function setChorus({mix,rate,depth}){ if(!fx)return; const c=getCtx(),n=c.currentTime;
  ...
  function setPhaser({mix,rate,depth}){ if(!fx)return; const c=getCtx(),n=c.currentTime;
    if(mix!=null) fx.phaser.wet.gain.setTargetAtTime(mix,n,0.05);
  ...
  function setReverb({mix,size}){ if(!fx)return; const c=getCtx(),n=c.currentTime;
    if(mix!=null) fx.reverb.wet.gain.setTargetAtTime(mix,n,0.05);
    if(size!=null) fx.reverb.conv.buffer=makeImpulse(c, 0.5+size*3.5, 2.5);
  }
  function setPresence(db){ if(!fx)return; const c=getCtx();
    fx.shelf.gain.setTargetAtTime(db, c.currentTime, 0.05);
  }

  // route a playback voice to wet (true) or dry (false) bus
  ...
  function start(waveform) {
    ...
    for (let i = 0; i < MAXV; i++) {
      ...
      gain.gain.value = 0;
      osc.connect(gain);
      gain.connect(master); // live voices → wet bus
      osc.start();
      warbleOsc.start();
  ...
  function setWaveform(w) { voices.forEach(v => v.osc.type = w); }

  // Set frequency of one voice with optional glide (seconds). 0 = instant.
  ...
  // amplitude per voice (0..1). number of active voices auto-balanced by caller.
  function setVoiceGain(i, g) {
    const v = voices[i]; if (!v) return;
    const c = getCtx();
    v.gain.gain.setTargetAtTime(Math.max(0, Math.min(1, g)), c.currentTime, 0.015);
  }
  ...
  // Re-assert every live voice's frequency + a tiny gain floor against a freshly
  // read clock. Used right after a context resume so ramps don't sit on a stale
  // currentTime (the iOS "silent until second gesture" race).
  function kick() {
    if (!started) return;
    const c = getCtx(), now = c.currentTime;
    voices.forEach(v=>{
  ...
  // ── Playback bank: a pool of voices for replaying recorded loops ──
  // Each playing loop "leases" up to 5 voices. We keep a flat pool and a map.
  const PB_VOICES = 40; // 8 slots * 5 voices worst case
  const pb = [];        // {osc, warbleOsc, warbleGain, gain, type}
  let pbStarted = false;
  function pbStart() {
    const c = getCtx();
    ensureBuses();
    ...
    for (let i = 0; i < PB_VOICES; i++) {
      ...
      osc.connect(gn); gn.connect(wetIn); // default: through effects
      osc.start(); wo.start();
  ...
  function pbSet(idx, { freq, gain, wave, warbleRate=0, warbleDepth=0 }) {
    ...
    const c = getCtx(), now = c.currentTime;
    if (wave && v.osc.type !== wave) v.osc.type = wave;
  ...
  function pbSilence(idx){ const v=pb[idx]; if(v){ const c=getCtx(); v.gain.gain.setTargetAtTime(0,c.currentTime,0.02);} }
  function pbSilenceAll(){ for(let i=0;i<pb.length;i++) pbSilence(i); }

  // one-shot preview note: routes through wet bus so effects apply
  ...
  return { getCtx, start, stop, setWaveform, setVoiceFreq, setVoiceGain,
           setVoiceWarble, silenceAll, kick, click,
           pbStart, pbEnsureMaster, pbSet, pbSilence, pbSilenceAll, pbRoute, PB_VOICES,
           setDelay, setChorus, setPhaser, setReverb, setPresence, ensureBuses, previewNote,
           isStarted: () => started, hasBuses: () => busesReady, unlock, MAXV };
}
...
// ─── Component ───────────────────────────────────────────────────────────────
export default function Aetherharp() {
  ...
  const pushFxParams = useCallback((fp)=>{
    ...
    const d=fp.delay, ch=fp.chorus, ph=fp.phaser, rv=fp.reverb, pr=fp.presence;
  ...
  const saveMelody = useCallback(()=>{
    const data = {
      version: 2, kind: "aetherharp-melody",
      bpm, beatsPerMeasure, measures,
    ...
    a.href=url; a.download="aetherharp-melody.json"; a.click();
    ...
    input.onchange=(e)=>{
      ...
      reader.onload=(ev)=>{
        try{
          ...
          if(d.kind!=="aetherharp-melody"){ setMelodyStatus("not a melody!"); setTimeout(()=>setMelodyStatus(""),2200); return; }
  ...
  useEffect(()=>{
    ...
    const h=(e)=>{ if(e.data?.target!=="aetherharp")return;
    ...
    <div style={{height:"100vh",maxHeight:"100vh",overflow:"hidden",
      ...
      <div style={{position:"fixed",left:0,right:0,bottom:0,zIndex:20,
        ...
        <div style={{width:"100%",maxWidth:"760px",pointerEvents:settingsOut?"auto":"none",
          ...
          {/* waveform */}
          <div style={{display:"flex",gap:"6px",alignItems:"center",marginBottom:"12px",flexWrap:"nowrap"}}>
            <Label>VOICE</Label>
            {WAVEFORMS.map(w=>(
      ...
      <div style={{position:"fixed",left:0,right:0,bottom:0,zIndex:22,
        ...
        <div style={{width:"100%",maxWidth:"760px",pointerEvents:fxOut?"auto":"none",
          ...
          <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:"9px",color:`${BRASS}66`,marginTop:"4px"}}>
            High-shelf at 3.5 kHz · delay → chorus → phaser → reverb
          </div>
        </div>
      </div>
...
// ─── Autoharp keyboard (3 octaves, horizontal, display-only, sage/gray) ──────────
